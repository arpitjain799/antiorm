...
class MormConnectionEngine(MormEngine):
    """
    Class that provides access to a persistent network connection.  This does
    *not* manage connection pooling at all, it is just a way to create cursors
...
class MormTable(object):
    ...
    def delete( cls, condstr=None, condargs=None, cursor=None ):
        ...
        if cursor is None:
            ...
            conn = engine.acquire_connection()
            cursor = conn.cursor()
        try:
    ...
    def getsequence( cls, pkseq=None, cursor=None ):
        ...
        if cursor is None:
            engine = cls.getengine()
            conn = engine.acquire_connection_ro()
            cursor = conn.cursor()
        try:
...
FIXME FIXME this is n*xed up, we need to pass in the connection that we will use
for the transaction, because it cannot be released, it needs remain open for the
duration of the execute and beyond.  This means that we cannot manage stuff with
an engine, unless we return some kind of wrapper object.  Nix it, oblige the
user to pass in a connection for every action.  Do we need for him to be able to
pass a cursor?
